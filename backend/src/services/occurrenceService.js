...
/**
 * Executa a verificação de ocorrências para um único estágio específico.
 */
async function checkSingleInternshipOccurrences(internshipId, userId) {
    console.log(`[OCCURRENCE CHECK] Iniciando verificação para o estágio ${internshipId}...`);
    try {
        if (userId) {
            try {
                const description = `Disparo por alteração de status - Usuário: ${userId} - Estágio: ${internshipId}`;
                await db.insert(keepAlive).values({ description });
                console.log(`[OCCURRENCE CHECK] Registro de keep_alive inserido: "${description}"`);
            } catch (keepAliveErr) {
                console.error('[OCCURRENCE CHECK] Erro ao inserir registro de keep_alive:', keepAliveErr);
        ...
        // 2. Buscar o estágio específico
        const [internship] = await db.select()
            .from(internships)
            .where(and(eq(internships.id, internshipId), isNull(internships.deletedAt)));

        if (!internship) {
            console.log(`[OCCURRENCE CHECK] Estágio ${internshipId} não encontrado ou foi excluído.`);
            return;
        }

        // 3. Buscar todas as ocorrências ativas deste estágio (não resolvidas)
        const openOccurrences = await db.select()
            .from(internshipOccurrences)
            .where(and(
        ...
        // Mapear ocorrências ativas por `ruleKey` -> ocorrência
        const openOccMap = new Map(openOccurrences.map(occ => [occ.ruleKey, occ]));
        ...
        // Avaliar cada regra
        for (const rule of rules) {
            const existing = openOccMap.get(rule.key);
        ...
        // 5. Salvar novos alertas
        if (occurrencesToInsert.length > 0) {
            await db.insert(internshipOccurrences).values(occurrencesToInsert);
        }

        // 6. Resolver alertas antigos
        if (occurrenceIdsToResolve.length > 0) {
            await db.update(internshipOccurrences)
                .set({
                    resolvedAt: now,
        ...
        console.log(`[OCCURRENCE CHECK] Concluído para o estágio ${internshipId}. Novas: ${occurrencesToInsert.length}, Auto-resolvidas: ${occurrenceIdsToResolve.length}`);
    } catch (error) {
        console.error(`[OCCURRENCE CHECK ERROR] Falha ao executar verificação para o estágio ${internshipId}:`, error);
    }
}

module.exports = {
    checkAllOccurrences,
    checkSingleInternshipOccurrences
};
